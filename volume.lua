...
volume_widget:connect_signal('volume_change', function()
    awful.spawn.easy_async(request_command, 
    function(stdout)
        local currentVolume = parseAlsaOutput(stdout).volume
        naughty.notify({text = 'Volume: ' ..  currentVolume .. '%'})
    end)
end)

volume_widget:connect_signal('volume_mute', function()
    awful.spawn.easy_async(request_command,
    function(stdout)
        local currentMute = parseAlsaOutput(stdout).mute
        naughty.notify({text = 'Mute: ' .. currentMute})
    end)
end)

function parseAlsaOutput(stdout)
    local mute = string.match(stdout, "%[(o%a+)%]")
    local volume = string.match(stdout, "(%d?%d?%d)%%")
